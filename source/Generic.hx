...
class Generic {
	...
	*/
	...
	static function onCrash(u:UncaughtErrorEvent) {
	...
	public static function trace(thing:Dynamic, var_name:String, alert:Bool = false) {
		...
		var fp:String = "logs/" + var_name + dateNow + ".txt";
	...
	public static function copyContent(copyPath:String, savePath:String, isvideo:Bool = false)
	{
			trace(returnPath());
			...
			trace('FileSystem.exists(savePath) = ' + FileSystem.exists(savePath));
			trace('Assets.exists(copyPath) = ' + Assets.exists(copyPath));

			if(isvideo){
			if (!FileSystem.exists(savePath)/* && Assets.exists(copyPath)*/) {
				File.saveBytes(savePath, Assets.getBytes('videos:' + copyPath));
			    trace('saved');
			}
		    }else{
			if (!FileSystem.exists(savePath)/* && Assets.exists(copyPath)*/) {
